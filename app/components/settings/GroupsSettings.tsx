...
export default function GroupsSettings({ groups }: { groups: Category[] }) {
  const router = useRouter()
  return (
    <section className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
      <h2 className="text-xl font-bold mb-6 text-gray-900 dark:text-gray-100">Categories</h2>

      {/* Create Category Form */}
      <div className="mb-8">
        <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">Add New Category</h3>
        <form action={async (formData) => { await createCategory(formData); router.refresh() }} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-gray-200 dark:border-slate-600">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Category Name</label>
              <input 
                name="title" 
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Core Type</label>
              <select 
                name="type" 
          ...
          <SubmitButton pendingText="Adding Category...">
            Add Category
          </SubmitButton>
        </form>
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-slate-700 pt-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">Your Categories</h3>
